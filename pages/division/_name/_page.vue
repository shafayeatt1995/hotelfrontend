...
export default {
  ...
  auth: false,
  head() {
    const title = `visiting place in ${this.name} Division - Page ${this.page} | ${this.$pageTitle}`;
    const description = `Explore our ${this.name} division featuring insightful articles and guides. Discover top destinations, tips, and more. page number ${this.page}.`;
    return {
      title,
  ...
  async asyncData(context) {
    try {
      ...
      let res = await axios.get(
        `${apiUrl}/api/fetch/posts-division/${name}/${page}`
      );
      if (res.data) {
