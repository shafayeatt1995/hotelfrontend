    ...
    >
      <div>
        <h2 class="text-3xl font-medium text-gray-700">
          <i class="fas fa-hotel"></i> Extra Service
        </h2>
      </div>
      ...
      >
        ...
        <template #empty v-if="items.length === 0 && !loading">
          <div class="flex items-center text-center h-96 bg-white">
            <EmptyMessage
              @action="modal = true"
              title="No Extra Service found"
              buttonText="Add Extra Service"
      ...
      <template #title>
        <div class="flex justify-between items-center">
          <h1
            ...
            id="modal-title"
          >
            {{ editMode ? "Edit" : "Create new" }} Extra Service
          </h1>
        ...
        <form class="mt-4" @submit.prevent="submit">
          ...
          <div class="mt-4 flex items-center gap-3">
            ...
            <Button type="submit" class="w-full tracking-wide flex-1">
              {{ editMode ? "Update" : "Create" }} Extra Service
            </Button>
          </div>
        </form>
      </template>
...
<script>
export default {
  name: "ExtraServicePage",
  layout: "dashboard",
  middleware: "owner",
  head() {
    return {
      title: `Extra Service - ${this.$pageTitle}`,
    };
  },
  data() {
    return {
  ...
  methods: {
    async fetchItems() {
      try {
        ...
        if (Number.isInteger(params.page)) {
          const { items } = await this.$ownerApi.fetchExtraService(params);
    ...
    async submit() {
      try {
        if (this.editMode) {
          await this.$ownerApi.updateExtraService(this.form);
        } else {
          await this.$ownerApi.createExtraService(this.form);
        }
        this.$nuxt.$emit(
          "success",
          `Extra service ${this.editMode ? "updated" : "created"} successfully`
        );
        this.refetch();
        this.modal = false;
      } catch (error) {
    ...
    async deleteItem(_id, key) {
      if (confirm("Are you sure, you want to delete?")) {
        try {
          await this.$ownerApi.deleteExtraService({ _id });
          this.items.splice(key, 1);
          this.$nuxt.$emit("success", "Extra service deleted successfully");
        } catch (error) {
          this.$nuxt.$emit(
            "error",
