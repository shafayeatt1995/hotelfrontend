    ...
    >
      <div>
        <h2 class="text-3xl font-medium text-gray-700">
          <i class="fas fa-hotel"></i> Facilities
        </h2>
      </div>
      ...
      <template #title>
        <div class="flex justify-between items-center">
          <h1
            ...
            id="modal-title"
          >
            {{ editMode ? "Edit" : "Create new" }} Facility
          </h1>
...
export default {
  ...
  methods: {
    async fetchItems() {
      try {
        ...
        if (Number.isInteger(params.page)) {
          const { items } = await this.$adminApi.fetchFacility(params);
    ...
    async submit() {
      try {
        if (this.editMode) {
          await this.$adminApi.updateFacility(this.form);
        } else {
          await this.$adminApi.createFacility(this.form);
        }
        this.$nuxt.$emit(
          "success",
          `Facility ${this.editMode ? "updated" : "created"} successfully`
        );
        this.refetch();
        this.modal = false;
      } catch (error) {
    ...
    async deleteItem(_id, key) {
      if (confirm("Are you sure, you want to delete?")) {
        try {
          await this.$adminApi.deleteFacility({ _id });
          this.items.splice(key, 1);
          this.$nuxt.$emit("success", "Facility deleted successfully");
        } catch (error) {
          this.$nuxt.$emit(
            "error",
    ...
    async toggleStatus({ _id }) {
      try {
        await this.$adminApi.toggleFacilityStatus({ _id });
